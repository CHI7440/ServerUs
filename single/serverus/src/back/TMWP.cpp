...
{
string value="";
map<string,string>::iterator it = this->requestMap->find(key);
if(it == this->requestMap->end()) return value;
return it->second;
}

void Request::setClientSocketDescriptor(int clientSocketDescriptor)
{
this->clientSocketDescriptor = clientSocketDescriptor;
}

int Request::getClientSocketDescriptor()
{
return this->clientSocketDescriptor;
}

void Request::setPortNumber(int portNumber)
{
this->portNumber= portNumber;
}

int Request::getPortNumber()
{
return this->portNumber;
}

void Request::forward(string resource)
{
/*TMWebProjector s(this->portNumber);
string requestBuffer = "GET /"+resource;
s.start(requestBuffer);*/
}
...
this->responseBuffer.append(header);
...
int Response::getClientSocketDescriptor()
{
return this->clientSocketDescriptor;
}
...
void Response::close()
{
send(this->clientSocketDescriptor,responseBuffer.c_str(),this->responseBuffer.length(),0);
...
int ii=0;
if(url[0]=='/') ii=1;
allRequests[string(url+ii)] = ptrOnRequest;
}
...
//int ii=0;
//if(this->url[0] == '/') ii=1;
//map me dhhondne wali cheez
//if(strcmp(this->url+ii,request->resource) == 0)
map<string,void(*)(Request&,Response&)>::iterator it = this->allRequests.find(string(request->resource));
if(it != this->allRequests.end())
{
rq.setMethod(request->method);
rq.setResource(request->resource);
rq.setMIMEType(request->mimeType);
rq.setClientSocketDescriptor(clientSocketDescriptor);
rq.setPortNumber(portNumber);
for(int x=0; x<request->dataCount; x++)
{
string key="";
string value="";
i=0;
while(request->data[x][i] != '\0')
{
key += request->data[x][i];
i++;
if(request->data[x][i]=='=')
{
key = URLEncoding::decode(key);
...
//sprintf(responseBuffer,"HTTP/1.1 200 OK\nContent-Type:text/html\nConnection:close\n\n");
//strcat(responseBuffer,rs.responseBuffer.c_str());
//send(clientSocketDescriptor,responseBuffer,strlen(responseBuffer),0);
//closesocket(clientSocketDescriptor);
}
else
{
printf("4 Sending 404 error page\n");
char tmp[501];
sprintf(tmp,"<!DOCTYPE HTML><html lang = 'en'><head><meta charset = 'utf-8'><title>My Web Projector</title></head><body><h1 style = 'color:red'>Resource /%s not found</h1></body></html>",request->resource);
sprintf(responseBuffer,"HTTP/1.1 200 OK\nContent-Type:text/html\nConnection:close\n\n");
strcat(responseBuffer,tmp);
