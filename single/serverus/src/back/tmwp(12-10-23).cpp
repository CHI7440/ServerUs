...
if(extensionEquals(resource+lastIndexOfDot+1,"sct"))
{
mimeType = (char *) malloc(sizeof(char)*9);
...
if(resource[i]=='.' && strcmp(getMimeType(resource),"text/sct")==0) return 'N';
return 'Y';
}
...
if(request->mimeType != NULL && strcmp(request->mimeType,"text/sct")==0)
{
FILE *f;
f = fopen(request->resource,"rb");
if(f==NULL)
...
{
cout<<"forwarded to client side resource"<<endl;
//closesocket(clientSocketDescriptor);
this->clientSide(request);
}
else
{
cout<<"forwarded to server side resource"<<endl;
//closesocket(clientSocketDescriptor);
